...
$(function () {
  var $sidebarContent = $('.sidebar').contents();
  var $header = $('.header-menu');
  $(window).on('resize', function () {
    if ($(window).width() <= 992) {
      if (!$sidebarContent.parent().is($header)) {
        $header.append($sidebarContent);
        $('.side-menu-name-link').on('click', function (e) {
          e.preventDefault();
...
$(document).ready(function () {
  ...
  $('.default-video-wrap video').each(function () {
    ...
    player.on('timeupdate', function (e) {
      var currentTime = e.detail.plyr.currentTime;
  ...
  function handleFileInputChange(inputId, avatarClass, infoClass, nameClass, changeClass, deleteWrapClass) {
    var initialState = {
      ...
      deleteWrapHtml: $("".concat(infoClass, " ").concat(deleteWrapClass)).html()
    };
    $(inputId).on('change', function (e) {
      if (this.files && this.files[0]) {
        ...
        reader.onload = function (e) {
          $(avatarClass).attr('src', e.target.result).css({
            "width": "100%",
            "height": "100%",
            "object-fit": "cover"
          });
          ...
          $(".info-add-photo-result ".concat(deleteWrapClass)).html("<a href=\"#\" class=\"delete-photo\"><img src=\"../../img/Trash.svg\" alt=\"delete\"></a>");
        };

        reader.readAsDataURL(this.files[0]);
        $(document).on('click', "".concat(changeClass, " .change-file-btn"), function () {
          $(inputId).click();
        });
        $(infoClass).on('click', "".concat(deleteWrapClass, " .delete-photo"), function (e) {
          e.preventDefault();
          $(avatarClass).attr('src', initialState.avatarSrc).css({
            "width": "auto",
            "height": "auto"
          });
          ...
          $("".concat(infoClass, " ").concat(deleteWrapClass)).html(initialState.deleteWrapHtml);
        });
      }
    });
  }
  ...
  function autoExpand(textarea) {
    var $textareaWrap = $(textarea).closest('.textarea-wrap');
  ...
  var dropAreaVideo = $('.drag-and-drop-video-wrap');
  var dropAreaImg = $('.drag-and-drop-img-wrap');

  function handleFiles(file, dropArea, fileType) {
    var rgxpFileType = new RegExp('^' + fileType + '/');
      ...
      reader.onload = function (e) {
        if (fileType == 'video') {
          setTimeout(function () {
            ...
            dropArea.find('.dropped-file-video-wrap').css({
              "z-index": "100"
            });
        ...
        dropArea.find('.dropped-file').attr('src', e.target.result).css({
          "z-index": "100"
        });
      };
  ...
  $('.subscription').on('click', function (e) {
    e.preventDefault();
    var subscriptionValue = $(this).text();
    $(this).text(subscriptionValue === "Subscribe" ? "Unsubscribe" : "Subscribe");
  });
  $('.show-more-post-description').on('click', function (e) {
    e.preventDefault();
    var ValueAction = $(this).text();
    $('.hidden-video-post-description').toggleClass('active');
    $(this).text(ValueAction === "Show more" ? "Show less" : "Show more"); // $(this).closest('.item-video-post-description').find('.item-video-post-description-text').html(``)
  });
  var $mainComment = $('.main-comment');
  var $demonstrativeComment = $('.replies-comment');
  ...
  var defaultText = $('.btn-show-more-user').text();
  ...
  $('.btn-search-header').on('click', function (e) {
    e.preventDefault();
    var inputWrapSearch = $(this).closest('.input-wrap-search:not(.active)');
  ...
  $(document).on('click', function (e) {
    if (!$(e.target).closest('.input-wrap-search').length) {
      var activeInputWrap = $('.input-wrap-search.active');
